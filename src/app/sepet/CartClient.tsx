...
export function CartClient({ targetTimeMs, message, shippingFee, shippingThreshold }: { targetTimeMs: number, message: string, shippingFee: number, shippingThreshold: number }) {
  ...
  if (!mounted) return <div className="py-20 text-center text-neutral-500">Yükleniyor...</div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div className="md:col-span-2 space-y-4">
        ...
        {cart.length === 0 ? (
          ...
          <div className="bg-neutral-0 rounded-xl border border-neutral-200 shadow-[var(--shadow-card)] overflow-hidden">
            {cart.map((item, i) => (
              <div key={item.id + i} className="p-4 border-b border-neutral-200 last:border-0 flex items-center gap-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-neutral-50 rounded flex items-center justify-center flex-shrink-0">
                  {item.image ? (
                    <img src={item.image} alt={item.title} className="max-w-full max-h-full object-contain p-2 mix-blend-multiply" />
                  ) : (
                    <span className="text-[10px] text-neutral-200">Görsel</span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <Link href={`/urun/${item.slug}`} className="text-xs sm:text-sm font-semibold text-neutral-900 hover:text-trust-blue-600 line-clamp-2">
                    {item.title}
                  </Link>
                ...
                <div className="flex flex-col items-end gap-2">
                  <button onClick={() => removeItem(item.id)} className="text-neutral-500 hover:text-risk-red-500 transition-colors">
                    <Trash2 size={16} className="sm:w-5 sm:h-5" />
                  </button>
                  <div className="flex items-center border border-neutral-200 rounded-lg">
                    <button onClick={() => updateQty(item.id, -1)} className="px-2 py-1 text-neutral-500 hover:bg-neutral-100 rounded-l-lg"><Minus size={12} className="sm:w-3.5 sm:h-3.5" /></button>
                    <span className="px-1 sm:px-2 py-1 text-xs sm:text-sm font-bold min-w-[1.5rem] sm:min-w-[2rem] text-center">{item.qty}</span>
                    <button onClick={() => updateQty(item.id, 1)} className="px-2 py-1 text-neutral-500 hover:bg-neutral-100 rounded-r-lg"><Plus size={12} className="sm:w-3.5 sm:h-3.5" /></button>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="md:col-span-1">
        <div className="bg-neutral-50 p-6 rounded-xl border border-neutral-200 sticky top-24">
          <h2 className="text-lg font-bold text-neutral-900 mb-4">Sipariş Özeti</h2>

          <div className="flex justify-between mb-2 text-neutral-500 text-sm sm:text-base">
          ...
          <div className="flex justify-between mb-4 text-neutral-500 text-sm sm:text-base">
            <span>Kargo</span>
            ...
            <div className="mb-4">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 border border-neutral-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-trust-blue-500 outline-none"
                />
                <button onClick={applyCoupon} disabled={applyingCoupon} className="bg-trust-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-trust-blue-600/90 transition-colors disabled:opacity-50">
                  Uygula
                </button>
              </div>
              {couponError && <p className="text-risk-red-500 text-xs mt-1">{couponError}</p>}
            </div>
          )}
          ...
          <div className="flex justify-between mb-6 text-lg sm:text-xl font-bold text-neutral-900">
            <span>Toplam</span>
            <span>{finalTotal.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL</span>
          </div>

          {cartTotal > 0 && neededForFreeShipping > 0 && (
            <div className="mb-6 p-3 bg-risk-red-500/10 text-risk-red-500 text-xs sm:text-sm font-semibold rounded-lg text-center border border-risk-red-500/20">
          ...
          <Link
            ...
            className={`w-full py-3 sm:py-4 rounded-xl font-bold flex justify-center items-center transition-all duration-normal shadow-[var(--shadow-button)]
              ${cartTotal === 0 ? 'bg-neutral-200 text-neutral-500 pointer-events-none' : 'bg-cta-background hover:bg-cta-hover text-white hover:shadow-[var(--shadow-card-hover)]'}
            `}
          >
            Ödemeye Geç <ArrowRight size={18} className="ml-2" />
          </Link>

          <div className="mt-4 flex items-center justify-center text-[10px] sm:text-xs text-neutral-500">
